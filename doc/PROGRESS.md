...
## 当前状态：✅ 阶段1-3核心完成，阶段4方案已确认 🎉

> **最新调整**（2026-05-12）：论坛帖子相关能力已基本可用，后续进入遗留 Bug 优化阶段；同时正式启动阶段4“球馆 / 俱乐部 / 活动大厅”系统设计。客户端文档见 `doc/05_Phase4_球馆俱乐部大厅设计.md`，服务端文档见 `yucircle-server/doc/05_Phase4_球馆俱乐部大厅_服务端开发.md`。
...
**阶段4 - 球馆 / 俱乐部 / 活动大厅系统 ✅ 2026-05-12 后端完成**
- ✅ 业务模式确认：球馆 -> 俱乐部 -> 活动大厅 三层结构
- ✅ MVP 固定测试数据确认：
  - ✅ 球馆：晴天羽毛球馆 (10个场地)
...
**客户端文档已输出**：
- ✅ `doc/05_Phase4_球馆俱乐部大厅设计.md` - Flutter UI 设计指南

**服务端已完成 ✅ (2026-05-12 17:30)**:
- ✅ **架构完成**：Entity + Mapper + DTO + Service + Controller 三层
- ✅ **7个核心Entity**：Venue, Club, Activity, ActivitySlot, ActivityPresence, ActivityMessage, Broadcast
- ✅ **13个API端点全部实现**：
  - 场馆管理：GET /venues (1个)
  - 俱乐部：GET/POST /clubs/* (3个)
  - 活动大厅核心（**重点**）：
    - ✅ POST /activities/{id}/enter - 进入
    - ✅ GET /activities/{id}/lobby - 大厅快照 (核心端点)
    - ✅ POST /activities/{id}/ping - 心跳 (2秒轮询)
    - ✅ POST/DELETE /activities/{id}/reserve - 预订 (3分钟倒计时)
    - ✅ POST /activities/{id}/confirm - 确认 (6人满自动锁定)
    - ✅ POST /activities/{id}/messages - 聊天 (60秒速率限制)
    - ✅ POST /activities/{id}/leave - 离开
  - 广播：GET/POST /broadcasts/* (2个)
- ✅ **关键功能已实现**：
  - JWT Token 身份验证 (所有10个TODO已修复)
  - 场地预订流程 (reserved → confirmed → locked)
  - 消息60秒速率限制
  - 自动清理任务 (presence/slot/message/broadcast)
  - 俱乐部实时统计 (todayActivityCount, onlineCount, hasBroadcast)
- ✅ **数据库初始化**：完整 SQL 脚本，包含测试数据
- ✅ **编译通过**：`BUILD SUCCESSFUL`
- ✅ **应用启动**：服务端运行于 `http://localhost:8080/api`
- ✅ **API 测试**：初步验证 `GET /venues` 端点可用

**客户端待开发**:
- ⏳ 底部导航改造 (Tab 2 = 场馆，从发现页改为场馆列表)
- ⏳ 球馆列表页面
- ⏳ 俱乐部列表页面 (实时统计显示)
- ⏳ 活动大厅页面 (核心UI)
  - 球场网格 (2x5 或响应式)
...
**集成测试文档已生成 ✅**:
- ✅ `doc/06_Phase4_集成测试清单与客户端开发指南.md`
- 内容包括：
  - API 快速检查表 (13个端点)
  - JWT Token 使用说明
  - 7个完整测试场景 (从基础查询到多球场)
  - 客户端10大检查清单 (Tab导航、心跳、预订、消息等)
  - 关键技术注意事项 (轮询间隔、状态转移等)
  - 监控指标与快速开始示例代码
  - 问题排查指南
...
### Phase 4：球馆 / 俱乐部 / 活动大厅系统 🚀

**优先级**：高（论坛之后的核心业务模块）

**客户端设计文档**：`doc/05_Phase4_球馆俱乐部大厅设计.md`

**服务端设计文档**：`yucircle-server/doc/05_Phase4_球馆俱乐部大厅_服务端开发.md`

**业务目标**：
- [ ] 建立“球馆 -> 俱乐部 -> 活动大厅”的打球组织链路
- [ ] 让用户能直观看到某个大厅里有多少人在线、哪些人已占坑、哪些人已确认
- [ ] 支持俱乐部广播补位，形成更强的组局氛围
- [ ] 支持 6 人确认成局，便于球馆老板按先后顺序分配真实场地

**客户端任务**：
- [ ] 调整底部导航：Tab1 改为球馆，Tab2 改为社区
- [ ] 创建球馆列表页：展示晴天羽毛球馆、距离占位、在线人数、广播入口
- [ ] 创建俱乐部列表页：展示俱乐部 A / B / C 的活动状态与在线人数
- [ ] 创建活动大厅页：
  - [ ] 顶部信息区（球馆、俱乐部、时间段、在线人数）
  - [ ] 广播跑马灯区域
  - [ ] 10 个场地的可视化坑位区
  - [ ] 观望区
  - [ ] 聊天区
  - [ ] 输入与操作区
- [ ] 新增大厅状态管理 Controller（GetX）
- [ ] 新增大厅模型与接口适配
- [ ] 支持大厅 2 秒轮询、30 秒心跳保活
- [ ] 支持占坑、取消占坑、确认参加、发送消息

**服务端任务**：
- [ ] 新增球馆、俱乐部、活动、大厅坑位、在线记录、聊天消息、广播等实体
- [ ] 新增数据库迁移脚本：球馆 / 俱乐部 / 活动大厅相关表
- [ ] 初始化 MVP 数据：
  - [ ] 晴天羽毛球馆
  - [ ] 10 个场地
  - [ ] 俱乐部 A / B / C
...
- [ ] 实现大厅聊天接口（60 秒限频）
- [ ] 实现俱乐部广播接口与有效期控制
- [ ] 实现定时清理任务（过期广播、历史消息、超时在线记录）

**验收标准**：
- [ ] App 中能看到晴天羽毛球馆
- [ ] 点击球馆能进入俱乐部列表
- [ ] 点击俱乐部能进入大厅
- [ ] 大厅能正确展示 10 个场地与 6 个坑位
- [ ] 占坑、确认、成局状态流转正确
- [ ] 广播展示正常
- [ ] 聊天发送和限频正常
- [ ] App 端根据客户端文档可独立推进开发
- [ ] Server 端根据服务端文档可独立推进开发
...
- **2026-05-12**: 拆分阶段4客户端 / 服务端文档，新增 `05_Phase4_球馆俱乐部大厅设计.md`（客户端）与 `yucircle-server/doc/05_Phase4_球馆俱乐部大厅_服务端开发.md`（服务端）
